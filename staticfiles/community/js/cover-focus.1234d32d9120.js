 ...
 */
(function () {
    const COVER_RATIO = 2.4; // width / height — keep in sync with community.css

    const clamp = (value, min, max) => Math.min(max, Math.max(min, value));

    const visibleFractions = (imageWidth, imageHeight) => {
        const imageAR = imageWidth / imageHeight;
        if (imageAR >= COVER_RATIO) {
            // Wider than the cover frame: full height is kept, sides are cropped.
            return { vw: COVER_RATIO / imageAR, vh: 1 };
        }
        // Taller/narrower: full width is kept, top/bottom are cropped.
        return { vw: 1, vh: imageAR / COVER_RATIO };
    };
    ...
    const initCoverFocusPicker = () => {
        ...
        const fileInput = document.querySelector("[data-cover-focus-input]");
        const previewImage = document.getElementById("cover-focus-preview");
        const focusXInput = document.getElementById("id_cover_focus_x");
        const focusYInput = document.getElementById("id_cover_focus_y");
        ...
        const layout = () => {
            ...
            ({ vw, vh } = visibleFractions(imageWidth, imageHeight));
