...
def validate_event_poster(value):
    """
    Validate poster uploads on new files only.

    Existing Cloudinary ImageField values are stored as public IDs without a
    file extension; re-validating them would break edits that keep the image.
    """
    if value is None:
        return
...
def validate_event_poster_upload(uploaded_file):
    ...
    if not content_type.startswith("image/") and not is_heic_upload(uploaded_file):
        raise ValidationError("Only image files are allowed for event posters.")
